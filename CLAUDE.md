...
- **Build Cowork bundle:** `python scripts/build_cowork_plugin.py` → packages every skill into `claude-skills.plugin` in the sibling `Cowork Skills Library/` folder, for installing in Cowork. Run on every skill change. (A git `post-commit` hook can automate this, if installed with the user's consent.)
