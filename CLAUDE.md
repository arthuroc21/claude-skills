...
- **Build Cowork plugins:** `python scripts/build_cowork_plugin.py` → packages **each skill into its own `<skill>.plugin`** in the sibling `Cowork Skills Library/` folder, for installing individually in Cowork (each shows as `<skill>:<skill>`). New skills are auto-included; deleted ones have their stale `.plugin` removed. Run on every skill change. (A git `post-commit` hook can automate this, with the user's consent.)
