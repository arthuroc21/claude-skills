...
| scripts/build_cowork_plugin.py | scripts/ | Packages **each** repo skill into its own `<skill>.plugin`; writes them + an index README to the sibling `Skills Library/` (removes stale plugins). **Worktree-safe:** always targets the library beside the *main* checkout (via `git --git-common-dir`) |
