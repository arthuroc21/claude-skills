...
| scripts/build_cowork_plugin.py | scripts/ | Packages every repo skill into `claude-skills.plugin`; writes it + an index README to the sibling `Cowork Skills Library/` |
| Cowork Skills Library/ | `G:\…\Discovering Claude\Cowork Skills Library` (outside repo, on Drive) | Distribution folder: `claude-skills.plugin` (install-all-at-once Cowork bundle) + index README; rebuilt on every skill change |
...
- [ ] In Cowork: install `Cowork Skills Library/claude-skills.plugin` (attach in a Cowork chat → Accept). Reinstall to apply later updates.
- [ ] (Optional) Approve a git `post-commit` hook to auto-rebuild the Cowork bundle on every commit (today it's rebuilt via the sync convention / `scripts/build_cowork_plugin.py`).
